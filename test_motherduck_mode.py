...
for table, (min_count, max_count) in expected_counts.items():
    ...
    assert min_count <= count <= max_count, (
        f"{table} count {count} outside expected range [{min_count}, {max_count}]"
    )
...
print(
    f"[OK] Retrieved measure {first_measure_id}: {single_measure.get('measure', 'N/A')[:50]}..."
)
...
for view_name, min_count, max_count in view_tests:
    try:
        ...
        assert min_count <= count <= max_count, (
            f"{view_name} count {count} outside expected range"
        )
...
print("[OK] All queries executed against cloud database successfully")
print("[OK] Data persistence verified (all reads/writes go to cloud)")
...
print("[OK] Read Operations: Working")
print("[OK] Write Operations (Create/Update/Delete): Working")
print("[OK] Macros: Working")
print("[OK] Views: Working")
